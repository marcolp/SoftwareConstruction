...
public class Node {
	...
	private ArrayList<Node> connectedTo; // Node at index 0 should be the inside
											// of a block, index 1 should be
											// outside the block
	private nodeType type;
	private boolean isExitNode; // This means that this is where an
								// 'if'/'for'/etc ends up in
	private boolean isBlockEnd; // This means that this node is the last one in
								// a {} block
	...
	public Node getLastInnerChild() {
		...
		Node currentNode = this.getConnectedTo().get(0);
	...
	/**
	 * If the current node has a child that has more than 1 child (e.g. a LOOP
	 * node, IF node, etc) then find the last inner child of that node.
	 * 
	 * @return The
	 */
	public Node getLastOuterChild() {
		...
		if (this.connectedTo.size() > 1)
			...
			// System.out.println("Trying to traverse a node with not enough
			// children");
			return null;
		}
	...
	public Node getExitNode() {
		...
		while (!currentNode.connectedTo.isEmpty()) {
			if (currentNode.isExitNode() == true)
				return currentNode;
			if (currentNode.connectedTo.size() > 1)
				currentNode = currentNode.getConnectedTo().get(1);
	...
	// Find the last node (a node with less connections than it should have
	public Node findLastNode() {

		Node temp = this;
		if (this.type == Node.nodeType.NORMAL) {
			if (this.getConnectedTo().isEmpty())
				return this;
			else
				return this.getConnectedTo().get(0).findLastNode();
		}

		else {
			if(this.getConnectedTo().size() > 1) return this.getConnectedTo().get(1).findLastNode();
			else return this;
		}
